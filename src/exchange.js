...
const { loadOrders,loadSellOrders, loadTrade, saveTrade, saveOder, saveOderStrategy, loadOrderStrategy, getFreeBalance, loadMarkets, auth, publish } = require('common');
...
async function controlTrade(trade) {
    try {
        ...
        const strategy = await loadOrderStrategy(trade);
        if (strategy) {
