...
async function buyOder({
  ...
  if (!trade) {
    const orders = _.filter(
      await loadOrders(),
      o =>
        o.newClientOrderId.includes(strategy) &&
        o.newClientOrderId.includes(exchangeId)
    );
    ...
    if (!order && ordersCount < MAX_TRADE_COUNT_PER_STRATEGY) {
      ...
      if (tradeBalance) {
        ...
        if (quantity) {
          ...
          // publish(`newOrder`, order.newClientOrderId);
...
async function controlTrade(trade) {
  try {
    ...
    const strategy = (trade.strategy =
      trade.strategy || (await loadOrderStrategy(trade)));
       
    if (strategy) {
      const { takeProfit, stopLoss, trailingStop } = strategy;
      if (takeProfit) {
        if (trade.change >= takeProfit) {
          exchange.createLimitSellOrder(market.symbol, quantity, lastPrice, {
            newClientOrderId
          });
          //control this sell
          return;
      ...
      if (stopLoss) {
        ...
        if (trade.change <= trade.stopLoss) {
          exchange.createMarketSellOrder(market.symbol, quantity, {
            newClientOrderId
          });
          return;
        }
